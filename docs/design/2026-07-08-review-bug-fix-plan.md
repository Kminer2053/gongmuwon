...
9개 클러스터 전량 구현·검증·커밋 완료. 최종 그린: **사이드카 pytest 344 · 데스크톱 vitest 221**.

| # | 내용 | 커밋 |
|---|------|------|
| ① | 문서작성 스킬 통일(organize→format→build, 트랜스크립트 컨텍스트·참고자료 반영) | (W1 커밋) |
| ② | 위키 링크 400 — read_page 경로 상대화 + 프론트 base 기준 resolve | (W1 커밋) |
| ③ | 검색 관련성 — QUERY_STOPWORDS·ASCII↔한글 분리·접미어 스톱워드 | (W2 커밋) |
| ④ | 마크다운 렌더 — 빈줄 사이 번호목록 연속 ol·전각/원문자 정규화 | 415631a |
| ⑤ | `[[위키링크]]` 클릭 렌더 + vault-root 상대 해석 | 95d921f |
| ⑥ | (1) 응답맥락 초기화 버튼/엔드포인트 f892f03 · (3) 핵심문서 미리보기 모달 f3f2283 | f892f03, f3f2283 |
| ⑦ | 분류 정규화 — 연도("2026년도")·사본·(안)·(1) 가족키 병합 | e6bf508 |
| ⑧ | 마법사 폴더 마커(□주요 등) 정리 + 카드 레이아웃 wrap | 8c8d8e0 |
| ⑨ | 인덱스·주제 페이지 판본 가족 접힘(대표 1건 + '버전 이력' 링크) | 8760daf |

### 검증 반영
- **E2E(실 gemma-4-E2B, 등록된 외부 서비스 연결)**: 문서작성을 격리 워크스페이스에서 실행 → 개조식(□◦-※) 위계·플레이스홀더/에코 부재 육안 확인 통과. 검색·위키 렌더(⑤⑨)·미리보기(⑥)는 LLM 비의존(결정적)이라 통합 테스트로 계약 고정.
- **발화조건 반영**: 업무대화 스킬 트리거 5종을 tips.ts(APP_TIPS)→컴포저 힌트 + 설치 대기화면(generate-nsis-tips.mjs→installer-hooks.nsh)에 반영(31b2141).
- **스킬 팁·설치 안내**: 문서작성/시행문/일정삭제/멀티인텐트/기능도움말 예시 노출.
