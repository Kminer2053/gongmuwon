...
- `apps/desktop`: 메뉴 순서, 입력 폼, 우측 승인/실행기록 패널, 주요 화면 전환, 문서 최종 저장 request/apply UI, 지식 검색/그래프 inspector, 파일정리 액션 버튼, manifest 기반 도구 화면, runtime badge/manual lifecycle/auto-restart 흐름이 동작한다.
- `apps/desktop/src-tauri`: sidecar runtime status 조회와 수동 시작/종료/재시작 command, exit cleanup, crash detect status가 동작한다.
...
- 검증 결과:
  ...
  - `npm --workspace apps/desktop run test` -> `10 passed`
  ...
  - `npm --workspace apps/desktop run test -- src/app.test.tsx` -> `10 passed`
...
1. 오프라인 설치 패키징과 운영 정책 문서화는 더 남아 있다.
