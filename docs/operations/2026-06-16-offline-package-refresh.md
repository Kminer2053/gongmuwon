...
- `START_INSTALL.bat`와 `VALIDATE_INSTALL.bat`가 생성된다.
...
`START_INSTALL.bat`는 초보 사용자를 위한 원클릭 진입점이다. 내부적으로 `install-gongmu-ai.ps1`를 실행한다.
...
- clean-account 또는 VM에서 `START_INSTALL.bat` 실행
- 설치된 Gongmu 앱 실행
- 업무엔진 health OK 확인
