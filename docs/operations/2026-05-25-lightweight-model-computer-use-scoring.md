...
- 컴퓨터유즈 배치 실행 인덱스 JSON: `docs/operations/generated/lightweight-model-computer-use-batch-index.json`
- 컴퓨터유즈 배치 실행 인덱스 Markdown: `docs/operations/generated/lightweight-model-computer-use-batch-index.md`
- 컴퓨터유즈 배치 실행팩 폴더: `docs/operations/generated/lightweight-model-computer-use-batches/`
...
- 컴퓨터유즈 커버리지 감사 JSON: `docs/operations/generated/lightweight-model-computer-use-coverage-audit.json`
- 컴퓨터유즈 커버리지 감사 Markdown: `docs/operations/generated/lightweight-model-computer-use-coverage-audit.md`
...
100개 전체 실행팩이 너무 커서 컴퓨터유즈 한 턴에 다루기 어렵다면 카테고리별 배치 실행팩을 생성한다.

```powershell
npm.cmd run qa:batches:lightweight
```

이 명령은 10개 카테고리별 배치 Markdown/JSON과 배치 인덱스를 만든다. 각 배치 결과 JSON을 만든 뒤에는 점수 스크립트의 `--merge-results` 옵션을 반복 지정해 합산할 수 있다.
...
node scripts/score-lightweight-model-test-run.mjs `
  ...
  --merge-results docs/operations/generated/batch-results/computer-use-batch-01-results.json `
  --merge-results docs/operations/generated/batch-results/computer-use-batch-02-results.json `
  --audit-coverage
```

컴퓨터유즈 결과 파일이 전체 목표 완료로 인정될 만큼 충분한지 감사하려면 아래 명령을 사용한다.

```powershell
npm.cmd run qa:audit:lightweight
```

이 명령은 미실시 시나리오 수, 카테고리 커버리지, 증거 누락, 실패/차단 항목을 별도 감사 리포트로 남긴다. 전체 100개 시나리오를 모두 실제 조작하기 전에는 `readyForCompletion=false`가 정상이다.

릴리스 게이트에서 미완료 상태를 실패로 처리하려면 직접 점수 스크립트에 `--fail-on-incomplete`를 추가한다.
