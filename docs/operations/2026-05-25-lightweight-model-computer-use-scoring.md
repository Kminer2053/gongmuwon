...
이 명령은 `/api/settings`의 런타임 정책과 실제 업무대화 1턴 응답을 확인해 `LMUX-02-01`, `LMUX-03-04`, `LMUX-03-10` 항목을 자동 채점한다. 전체 100개 시나리오를 대체하지는 않지만, Gemma 4 E2B 최적화 변경이 런타임 정책, Markdown 목록, 내부추론/모델 메타 노출 방지에 영향을 주는지 빠르게 확인하는 회귀 스모크다.
